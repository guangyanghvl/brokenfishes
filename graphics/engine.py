...
class Engine3D:
    ...
    def __select(self, event):
        ...
        if found != []:
            ...
            self.__axis = [self.screen.createLine(self.__axis[0], 'red'),
                           self.screen.createLine(self.__axis[1], 'green'),
                           self.screen.createLine(self.__axis[2], 'blue')]
    ...
    def writeRedPoints(self, redpoint):
        self.redpoint = []
        self.redpoint.append(graphics.vertex.Vertex(redpoint))

    def writeTriangles(self, triangles):
        self.triangles = []
        for triangle in triangles:
            if len(triangle) != 4:
                triangle.append('grey')

            self.triangles.append(graphics.face.Face(triangle))

    def __init__(self, points, pointRed, triangles, width=1000, height=700, distance=6, scale=100, title='3D',
                 background='white'):
        # object parameters
    ...
    def render(self):
        ...
        # draw the red point
        self.reddot = []
        for rpoint in self.redpoint:
            self.reddot.append(rpoint.flatten(self.scale, self.distance))
        #reddot.append(self.redpoint.flatten(self.scale, self.distance))
        #print(reddot[0][0])
        #print(reddot[0][1])
        self.screen.image.create_oval(self.reddot[0][0], self.reddot[0][1], self.reddot[0][0] + 10, self.reddot[0][1] + 10, fill='red',
                               tag='vBall')
        self.screen.image.pack()